...
export default function EditAndAddAttraction({ type }) {
  ...
  const [activeStep, setActiveStep] = useState(type=="new"?0:1);
  ...
  let attraction = id ? { ...attractions.find(x => x.Id == id) } : null;
  const onSubmit = (data) => {
    const o = { ...object }
    if (type == "new" || activeStep == 1)
      switch (activeStep) {
        case 1:
          data.Status = true;
          data.CategoryId = attraction ? attraction.CategoryId : category.Id;
          data.Date = new Date();
          data.ManagerId = user.Id;
          if (attraction && type == "edit") {
